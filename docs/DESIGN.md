...
- 2026-09-21: webhook ログはスペースに 1 画面だけ置き、狭い見方はすべて同じ画面のクエリにした(0.4.0):
  `?label=`(その webhook の呼び出し)と `?model=`(そのモデルの変更が起こした呼び出し。スペースの webhook は
  全モデルで発火するのでその分も入る)。2 つの select が現在の絞り込みを表示し、かつ設定手段でもある。
