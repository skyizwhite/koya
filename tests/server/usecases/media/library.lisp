...
(deftest store-and-remove
  (let ((media (store-upload "website" (png-bytes 10 20) :filename "../evil/../photo.png" :alt "A photo")))
    ...
    (testing "references"
      ...
      (create "website" (find-model "website" "blog") (parse-json (format nil "{\"title\": \"x\", \"cover\": \"~a\"}" (media-id media))))
      (create "website" (find-model "website" "blog") (parse-json (format nil "{\"title\": \"y\", \"body\": \"<img src=\\\"~a\\\">\"}" (media-url media))))
