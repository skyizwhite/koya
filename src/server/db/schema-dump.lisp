(defpackage #:koya-server/db/schema-dump
  (:use #:cl)
  ...
  (:export #:snapshot
           ...
           #:read-snapshot
           #:write-snapshot))
(in-package #:koya-server/db/schema-dump)

;;; The migrations are the schema's only definition; this reads the shape they
;;; add up to back out of SQLite and writes it to src/server/db/schema.sql, so
...
;;; tests/server/db.lisp fails if it drifts from the migrations.

(defun snapshot-path ()
  (asdf:system-relative-pathname "koya-server" "src/server/db/schema.sql"))
